...
local function OnSyringeTouch( self, ent )
    ...
    if touchTr.HitWorld then
        ...
        LAMBDA_TF2:StopParticlesNamed( self, "healshot_trail_red" )
        LAMBDA_TF2:StopParticlesNamed( self, "healshot_trail_blue" )
        LAMBDA_TF2:StopParticlesNamed( self, "critical_rocket_blue" )
        LAMBDA_TF2:StopParticlesNamed( self, "critical_rocket_red" ) 
...
table.Merge( _LAMBDAPLAYERSWEAPONS, {
    tf2_crusaders = {
        model = "models/lambdaplayers/tf2/weapons/w_crusaders_crossbow.mdl",
        origin = "Team Fortress 2",
        prettyname = "Crusader's Crossbow",
        holdtype = "crossbow",
        bonemerge = true,
        ...
        OnDeploy = function( self, wepent )
            ...
            wepent:SetWeaponAttribute( "Damage", 38 )
            wepent:SetWeaponAttribute( "RateOfFire", 0.105 )
            wepent:SetWeaponAttribute( "Animation", ACT_HL2MP_GESTURE_RANGE_ATTACK_SMG1 )
            wepent:SetWeaponAttribute( "Sound", ")weapons/crusaders_crossbow_shoot.wav" )
            wepent:SetWeaponAttribute( "CritSound", ")weapons/crusaders_crossbow_shoot_crit.wav" )
            wepent:SetWeaponAttribute( "UseRapidFireCrits", false )
        ...
        OnAttack = function( self, wepent, target )
            ...
            syringe:SetLocalVelocity( spawnAng:Forward() * 3000 )

            local critType = self:GetCritBoostType()
            if isCrit then critType = TF_CRIT_FULL end

            if critType == TF_CRIT_FULL then
                ParticleEffectAttach( "critical_rocket_" .. ( self.l_TF_TeamColor == 1 and "blue" or "red" ), PATTACH_ABSORIGIN_FOLLOW, syringe, 0 )
            end

            ParticleEffectAttach( "healshot_trail_" .. ( teamClr == 1 and "blue" or "red" ), PATTACH_ABSORIGIN_FOLLOW, syringe, 0 )
